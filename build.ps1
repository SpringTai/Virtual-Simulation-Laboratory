...
$environmentPython = Join-Path $projectRoot '.venv-dev/Scripts/python.exe'
...
try {
    ...
    & $environmentPython native/build.py
    ...
    & $environmentPython -m PyInstaller --noconfirm --distpath dist-v2.1 TensileLab.spec
    if ($LASTEXITCODE -ne 0) { throw 'PyInstaller build failed.' }
    $executable = Join-Path $projectRoot 'dist-v2.1/TensileLab/TensileLab.exe'
    if (-not (Test-Path -LiteralPath $executable)) { throw 'Packaged executable is missing.' }
    foreach ($filename in @('使用说明.txt', '模型与数据说明.md', '示例_读取实验数据.py', 'README.md', '验收记录-v2.1.md', '改造说明-v2.1.md')) {
        $document = Join-Path $projectRoot $filename
        if (Test-Path -LiteralPath $document) { Copy-Item -LiteralPath $document -Destination (Join-Path $projectRoot 'dist-v2.1/TensileLab') -Force }
    }
    $releaseRoot = Join-Path $projectRoot 'release-v2.1'
